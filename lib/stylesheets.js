...
//
//
// To load and parse external stylesheets, you need to provide one of the
// following options:
//
// baseDir -  Resolves all relative paths relative to this directory
// baseURL -  Resovles all relative paths relative to this URL
// resolve -  A function that will be called with a relative path and expected
//            to return either an HTTP/S URL or a filename
//
// With no option, resolves all relative paths relative to the current working
// directory.
//
// The following two are equivalent:
//
// new Stylesheets({ baseDir: '/home/me' });
// new Stylesheets({ resolve: Stylesheets.fileResolver('/home/me') });
...
module.exports = class Stylesheets {
  ...
  // Available options:
  //
  //  baseDir - Resolve all relative paths from this directory
  //  baseURL - Resolve all relative paths from this URL
  ...
  constructor(options) {
    ...
    if (options.baseDir)
      this.resolve = Stylesheets.fileResolver(options.baseDir);
    else if (options.baseURL)
      this.resolve = Stylesheets.urlResolver(options.baseURL);
    else if (options.resolve)
      this.resolve = options.resolve;
    else
      this.resolve = Stylesheets.fileResolver();
  }
