...
module.exports = function applyInline(dom, rules) {
  ...
  const matchers    = _.flatten(rules.map(matchRule));
...
function propertiesFromMatchers(element, matchers) {
  ...
  return _.flatten(propertiesToInline);
}
...
function propertiesFromElement(element) {
  ...
  if (styleAttribute) {
    ...
    const properties  = decls
      ...
      .reduce(addPropertyToMap, new Map());
    return properties;
  } else
    return new Map();
}
...
function addPropertyToMap(map, property) {
  ...
  map.set(priority.name, priority);
  return map;
}
