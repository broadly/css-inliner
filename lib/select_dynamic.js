...
module.exports = function selectDynamicRules(context) {
  ...
  const dynamicRules  = allRules.map(toDynamicRule)
                                .filter(rule => rule)
                                .map(toImportantRule);
...
// Transforms all rule declarations into `!important`.
function toImportantRule(rule) {
  if (rule.type === 'atrule') {
    if (rule.name === 'font-face')
      return rule;
    else {
      const rules = rule.nodes.map(toImportantRule);
      return rule.clone({ nodes: rules });
    }
  }
  else {
    const decls = rule.nodes && rule.nodes.map(toImportantDeclaration);
    return rule.clone({ nodes: decls });
  }
}


function toImportantDeclaration(decl) {
  return decl.clone({ important: true });
}
