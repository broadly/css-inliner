...
//
// Stash and restore require the context to have a retag function.
...
module.exports = {
  ...
  // in the document, stashes them in context.stashed, and replaces them in the
  // HTML source with special markers, returning context.html that is safe to
  // parse.
  //
  // Input:   context with html, optional retag
  ...
  stash(context) {
    ...
    if (retag) {
      ...
      const tags        = Immutable.Set( retag(template) );
      ...
      const stashTag    = stashTagsInto(stashed);
      ...
      const result      = context
        ...
        .set('stashed', Immutable.Map(stashed))
        .delete('retag');
      return result;
  ...
  restore(context) {
    ...
    if (stashed) {
      ...
      const restoreTag  = restoreTagsFrom(stashed);
...
function stashTagsInto(map) {
  ...
  return function(template, tag) {
    ...
    // Cheap trick to replace the same tag multiple times
