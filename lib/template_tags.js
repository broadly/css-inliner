...
module.exports = {
  ...
  stash(context) {
    ...
    if (readTags) {
      ...
      // Make life easy on implementors
      const template    = html.toString();
      const tags        = solveForNestedTags( readTags(template) );
      ...
      const withoutTags = tags.reduce(stashTag, template);
...
function solveForNestedTags(tags) {
  const set = Immutable.Set(tags);
  return set.sort(longestFirst);
}
