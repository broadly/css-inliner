...
module.exports = class Cache {
  ...
  constructor(options) {
    options = options || {};
    this._cache         = new Map();
    this._loadCSSAsync  = getLoadAsyncFromOptions(options);

    const processor     = postcss(options.plugins || []);
    this._parse         = processor.process.bind(processor);

    function getLoadAsyncFromOptions(options) {
      if (options.loadAsync)
        return options.loadAsync;
      else if (options.precompile)
        return options.precompile(options.directory);
      else
