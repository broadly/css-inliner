...
//          return loadLess(pathname).then(parseLess);
//        };
//      }
...
function loadAsync(directory, pathname) {
  try {
    const filename = resolveFilename(directory, pathname);
    debug(`Resolved relative pathname "${pathname}" to filename "${filename}"`);
...
function resolveFilename(directory, pathname) {
  assert(pathname && pathname.length, 'Pathname missing or empty string');
